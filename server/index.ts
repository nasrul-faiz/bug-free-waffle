...
app.post('/api/bot/pairing', ensureDashboardAuth, (req: any, res: any) => {
  try {
    const raw = req?.body && typeof req.body === 'object' ? req.body : {}
    const pairingMethod = String(raw?.pairingMethod || '').trim().toLowerCase()
    const phoneNumber = String(raw?.phoneNumber || '').trim()

    if (pairingMethod !== 'qr' && pairingMethod !== 'phone') {
      return res.status(400).json({ success: false, error: 'Invalid pairing method' })
    }

    if (pairingMethod === 'phone' && !phoneNumber) {
      return res.status(400).json({ success: false, error: 'Phone number is required for phone pairing' })
    }

    const nextPairingMethod = pairingMethod === 'phone' ? 'phone' : 'qr'
    const nextPhoneNumber = nextPairingMethod === 'phone' ? phoneNumber : null
